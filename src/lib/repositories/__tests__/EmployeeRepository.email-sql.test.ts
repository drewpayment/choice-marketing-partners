 ...
 * so the emitted Postgres SQL is asserted verbatim.
 */
...
describe('EmployeeRepository — emitted SQL', () => {
  ...
  describe('findEmailOwner', () => {
    ...
    it("excludes the edited employee's own login accounts under BOTH linkage conventions", async () => {
      ...
      expect(usersSql).toMatch(/not \(exists \(.*\) or \(.*"users"\."id" = \$\d+.*\)\)/)
    })
  ...
  describe('linked-login relation reuse', () => {
    it('uses uncorrelated semi-joins for the hasUser flag and filter', async () => {
      ...
      const semiJoinPattern =
        /\("employees"\."id" in \(select "users"\."id" from "users" where "users"\."id" > \$\d+\) or "employees"\."id" in \(select "employee_user"\."employee_id" from "employee_user"\)\)/
      ...
      expect(countSql).toMatch(semiJoinPattern)
      expect(pageSql).toMatch(semiJoinPattern)
      // ...and the flag in the select list uses the same shape.
      const semiJoinAsHasUser = new RegExp(semiJoinPattern.source + ' as "hasUser"')
      expect(pageSql).toMatch(semiJoinAsHasUser)
    ...
    it('keeps the correlated form for the per-row emulation id', async () => {
      ...
      expect(pageSql).toMatch(/order by "users"\."uid" asc limit \$\d+\) as "user_id"/)
    })
    ...
    it('uses the same semi-join for the withUserAccounts stat', async () => {
      ...
      expect(statsSql).toMatch(
        /"employees"\."id" in \(select "users"\."id" from "users" where "users"\."id" > \$\d+\)/
      )
